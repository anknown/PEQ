 ...
 *  
 **/
require_once(dirname(__FILE__) . '/log/Writer.php');
require_once(dirname(__FILE__) . '/log/writer/Std.php');

/*********************************************
 * format string，取自lighttpd文档
 * 前面标记 - 代表Base_Log库不支持
 * 行为不一致的，均有注释说明
 * 后面的 === 之后的，是Base_Log扩展的功能
  ====== ================================
  Option Description
  ====== ================================
  %%     a percent sign
    ...
    **/
    public static function getDataPath() 
    {
        if (self::$strDataPath == null) {
            self::$strDataPath = Base_AppEnv::getEnv('data');
        }
    ...
    public static function getInstance($app = null,$logType=null)
    {
        ...
        if (empty(self::$arrInstance[$app])) {
			self::$logToml = Base_Conf::getAppConf("log.toml");
            // 生成路径
            $logPath = self::getLogPath();
            if (self::$logToml['use_sub_dir'] == "1") {
            ...
            //get log format
            if (isset(self::$logToml['format'])) {
                $format = self::$logToml['format'];
            } else {
                $format = self::DEFAULT_FORMAT;
            }

            if (isset(self::$logToml['format_wf'])) {
                $format_wf = self::$logToml['format_wf'];
            } else {
                $format_wf = $format;
            }

            $log_conf = array(
                'level'         => intval(self::$logToml['level']),
                'auto_rotate'   => (self::$logToml['auto_rotate'] == '1'),
                'log_file'      => $log_file,
                'format'        => $format,
                'format_wf'     => $format_wf,
            );

            self::$arrInstance[$app] = new Base_Log($log_conf);
        }
    ...
    public static function genLogID()
    {
        ...
        if (getenv('HTTP_X_FW_LOG_ID') && intval(trim(getenv('HTTP_X_FW_LOG_ID'))) !== 0) {
            define('LOG_ID', trim(getenv('HTTP_X_FW_LOG_ID')));
        } else {
            $arr = gettimeofday();
    ...
    public static function getReqID()
    {
        ...
        if (getenv('HTTP_X_FW_REQ_ID') && intval(trim(getenv('HTTP_X_FW_REQ_ID'))) !== 0) {
            define('REQ_ID', trim(getenv('HTTP_X_FW_REQ_ID')));
        } else {
            define('REQ_ID', 0);
        }
        return REQ_ID;
    ...
    public function parseFormat($format) {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            switch($code) {
            case 'h':
                $action[] = "(defined('CLIENT_IP')? CLIENT_IP : Base_Log::getClientIp())";
                break;
            case 't':
            ...
            case 'e':
                ...
                break;
            case 'f':
                $action[] = 'Base_Log::$current_instance->current_file';
                break;
            case 'H':
            ...
            case 'L':
                $action[] = 'Base_Log::$current_instance->current_log_level';
                break;
            case 'N':
                $action[] = 'Base_Log::$current_instance->current_line';
                break;
            case 'E':
                $action[] = 'Base_Log::$current_instance->current_err_no';
                break;
            case 'l':
                $action[] = "Base_Log::genLogID()";
                break;
            case 'r':
                $action[] = "Base_Log::getReqID()";
                break;
            case 'u':
                ...
                $action[] = "((defined('CLIENT_IP') ? CLIENT_IP: Base_Log::getClientIp()) . ' ' . \$____user____['uid'] . ' ' . \$____user____['uname'])";
                break;
            case 'S':
                if ($param == '') {
                    $action[] = 'Base_Log::$current_instance->get_str_args()';
                } else {
                    $param_name = var_export($param, true);
                    if (!isset($prelim_done['S_'.$param_name])) {
                        $prelim[] = 
                            "if (isset(Base_Log::\$current_instance->current_args[$param_name])) {
                            \$____curargs____[$param_name] = Base_Log::\$current_instance->current_args[$param_name];
                            unset(Base_Log::\$current_instance->current_args[$param_name]);
                ...
                break;
            case 'M':
                $action[] = 'Base_Log::$current_instance->current_err_msg';
                break;
            case 'x':
                ...
                case 'err_no':
                case 'err_msg':
                    $action[] = 'Base_Log::$current_instance->current_'.$param;
                    break;
                case 'log_id':
                    $action[] = "Base_Log::genLogID()";
                    break;
                case 'req_id':
                    ...
                    break;
                case 'app':
                    $action[] = "Base_Log::getLastLogger()";
                    break;
                case 'function_param':
                    $action[] = 'Base_Log::flattenArgs(Base_Log::$current_instance->current_function_param)';
                    break;
                case 'argv':
                    $action[] = '(isset($GLOBALS["argv"])? Base_Log::flattenArgs($GLOBALS["argv"]) : \'\')';
                    break;
                case 'pid':
                    $action[] = false === stripos(PHP_OS, "win") ? 'posix_getpid()' : 'getmypid()';
                    break;
                case 'encoded_str_array':
                    $action[] = 'Base_Log::$current_instance->get_str_args_std()';
                    break;
                case 'cookie':
                    $action[] = "(isset(\$_SERVER['HTTP_COOKIE'])? \$_SERVER['HTTP_COOKIE'] : '')";
                    break;
     ...
     * @param Base_Log_Writer $writer
     */
    private static function registerWriter(Base_Log_Writer $writer) {
    ...
    public static function useLogerStdOut() {
        ...
        self::registerWriter(new Base_Log_Writer_Std());
        $used = true; 
    }
}
